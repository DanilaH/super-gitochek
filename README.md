...
**Start here in a new chat — read ALL THREE manual reports:** [the original hand-reviewed ideas journal](docs/GAME_IDEAS.md), [the fifth/sixth scan's manual findings](docs/GAME_IDEAS_SCAN_05_06.md), and [the follow-up deep dive on Firing Balls, 115, Boom Dots and false positives](docs/GAME_IDEAS_DEEP_DIVE_2026-09-19.md). The [decisions log](docs/DECISIONS.md) explains why these curated documents, together, are the durable manual record; `data/discovery-registry.json` records automatic discovery, **not** approval. The original journal's `204 / 276` scan statistics are a historical snapshot; the latest completed scan (2026-09-19) catalogued 539 repositories, with 10 pending. Neither count is a count of promising games. The follow-up deep dive reexamined already discovered repositories; it did **not** start another scan. Keep the curated record updated after meaningful manual reviews rather than relying on chat history or expiring Actions artifacts.
